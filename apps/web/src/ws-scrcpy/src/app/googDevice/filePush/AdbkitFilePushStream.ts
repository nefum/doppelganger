...
import {
  ...
  FilePushState,
} from "../../controlMessage/CommandControlMessage";
...
export class AdbkitFilePushStream extends FilePushStream {
  ...
  public sendEventAppend({
    ...
    channel.send(
      CommandControlMessage.createPushFileCommand(appendParams).toBuffer(),
    );
  }
  ...
  public sendEventFinish({ id }: { id: number }): void {
    ...
    channel.send(
      CommandControlMessage.createPushFileCommand(finishParams).toBuffer(),
    );
  }
  ...
  public sendEventNew({ id }: { id: number }): void {
    ...
    channel.send(
      CommandControlMessage.createPushFileCommand(newParams).toBuffer(),
    );
  }

  public sendEventStart({
    id,
    fileName,
    fileSize,
  }: {
    ...
    channel.send(
      CommandControlMessage.createPushFileCommand(startParams).toBuffer(),
    );
  }
