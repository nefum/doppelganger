...
export default class FilePushHandler implements DragEventListener {
  ...
  private async pushFile(file: File): Promise<void> {
    ...
    const [{ code: startResponseCode }, { reader, result }] = await Promise.all(
      [waitPromise, FilePushHandler.getStreamReader(file)],
    );
    ...
    const processData = async ({
      ...
      if (done || !value) {
        ...
        console.log(
          TAG,
          `File "${fileName}" uploaded in ${Date.now() - start}ms`,
        );
        return;
      }
  ...
  onStreamResponse = (response: PushResponse): void => {
    ...
    const id =
      code === FilePushResponseStatus.NEW_PUSH_ID
        ? FilePushHandler.REQUEST_NEW_PUSH_ID
        : response.id;
    const resolve = this.responseWaiter.get(id);
    if (!resolve) {
      console.warn(
        TAG,
        `Unexpected push id: "${id}", ${JSON.stringify(response)}`,
      );
      return;
    }
