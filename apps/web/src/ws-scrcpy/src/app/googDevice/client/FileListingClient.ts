...
import FilePushHandler, {
  DragAndPushListener,
  PushUpdateParams,
} from "../filePush/FilePushHandler";
...
export class FileListingClient
  extends ManagerClient<ParamsFileListing, never>
  implements DragAndPushListener
{
  ...
  constructor(params: ParamsFileListing) {
    ...
    const fragment = html`<div id="${this.wrapperId}" class="listing">
      ...
      <div id="${parentDirLinkBox}" class="quick-link-box">
        <a class="icon up" href="#!" ${FileListingClient.PROPERTY_NAME}="..">
          [parent]
        </a>
      </div>
      <div id="${rootDirLinkBox}" class="quick-link-box">
        <a
          class="icon dir"
          href="#!"
          ${FileListingClient.PROPERTY_NAME}="${rootPath}"
        >
          [root]
        </a>
      </div>
      <div id="${storageDirLinkBox}" class="quick-link-box">
        <a
          class="icon dir"
          href="#!"
          ${FileListingClient.PROPERTY_NAME}="${storagePath}/"
        >
          [storage]
        </a>
      </div>
      <div id="${tempDirLinkBox}" class="quick-link-box">
        <a
          class="icon dir"
          href="#!"
          ${FileListingClient.PROPERTY_NAME}="${tempPath}/"
        >
          [temp]
        </a>
      </div>
    ...
    if (wrapper) {
      wrapper.addEventListener("click", (e) => {
        ...
        if (newPath !== this.path) {
          const entryIdString = e.target.getAttribute(
            FileListingClient.PROPERTY_ENTRY_ID,
          );
  ...
  public onFilePushUpdate(data: PushUpdateParams): void {
    ...
    if (
      !upload ||
      document.getElementById(upload.anchor.id) !== upload.anchor
    ) {
  ...
  protected loadContent(
    ...
    if (
      ...
      !(this.ws instanceof Multiplexer)
    ) {
      return;
    }
  ...
  protected finishDownload(channel: Multiplexer): void {
    ...
    const file = new File(download.chunks, name, {
