...
export class DevtoolsClient extends ManagerClient<ParamsDevtools, never> {
  ...
  private static compareBrowsers = (
    a: RemoteBrowserInfo,
    b: RemoteBrowserInfo,
  ): number => {
  ...
  private createBrowserBlock(
    serial: string,
    info: RemoteBrowserInfo,
  ): HTMLDivElement {
    const { socket } = info;
  ...
  private createPageBlock(
    ...
    const absoluteAddress =
      page.devtoolsFrontendUrl && page.devtoolsFrontendUrl.startsWith("http");
    ...
    if (page.webSocketDebuggerUrl) {
      ...
      const base =
        "devtools://devtools/bundled/inspector.html?experiments=true&ws=";
