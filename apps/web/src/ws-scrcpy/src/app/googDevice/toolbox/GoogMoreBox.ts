...
export class GoogMoreBox {
  ...
  constructor(
    ...
    for (const [action, command] of codes.entries()) {
      ...
      if (action === ControlMessage.TYPE_CHANGE_STREAM_PARAMETERS) {
        ...
        GoogMoreBox.wrap(
          "div",
          [iFrameIntervalLabel, iFrameIntervalInput],
          innerDiv,
        );
    ...
    sendScreenPowerModeButton.onclick = () => {
      const message = CommandControlMessage.createSetScreenPowerModeCommand(
        screenPowerModeCheck.checked,
      );
      client.sendMessage(message);
    };
    GoogMoreBox.wrap(
      "p",
      [screenPowerModeCheck, screenPowerModeLabel, sendScreenPowerModeButton],
      moreBox,
      ["flex-center"],
    );
    ...
    GoogMoreBox.wrap("p", [qualityCheck, qualityLabel], moreBox, [
      "flex-center",
    ]);
  ...
  private fit = (): void => {
    const { width, height } =
      this.client.getMaxSize() || GoogMoreBox.defaultSize;
    if (this.maxWidthInput) {
      this.maxWidthInput.value = width.toString();
    }
    if (this.maxHeightInput) {
