...
export abstract class InteractionHandler {
  ...
  private static eventListeners: Map<string, Set<InteractionHandler>> =
    new Map();
  ...
  protected static bindGlobalListeners(
    interactionHandler: InteractionHandler,
  ): void {
    interactionHandler.touchEventsNames.forEach((eventName) => {
      let set: Set<InteractionHandler> | undefined =
        InteractionHandler.eventListeners.get(eventName);
      if (!set) {
        set = new Set();
  ...
  protected static onInteractionEvent = (
    event: MouseEvent | TouchEvent,
  ): void => {
    const set = InteractionHandler.eventListeners.get(
      event.type as TouchEventNames,
    );
    if (!set) {
      return;
    }
  ...
  protected static loadImages(): void {
    ...
    touch.src = TouchPointPNG.src;
    touch.onload = onload;
    const center = (this.centerPointImage = new Image());
    center.src = CenterPointPNG.src;
  ...
  private static validateMessage(
    ...
    if (action === MotionEvent.ACTION_UP) {
      if (!previous) {
        console.warn(
          logPrefix,
          "Received ACTION_UP while there are no DOWN stored",
        );
      } else {
      ...
      if (previous) {
        console.warn(
          logPrefix,
          "Received ACTION_DOWN while already has one stored",
        );
      } else {
      ...
      if (!previous) {
        if (
          ...
          console.warn(
            logPrefix,
            "Received ACTION_MOVE while there are no DOWN stored",
          );
          const emulated = InteractionHandler.createEmulatedMessage(
  ...
  public drawPointer(point: Point): void {
    this.drawPoint(
      point,
      InteractionHandler.touchPointRadius,
  ...
  public drawCenter(point: Point): void {
    this.drawPoint(
      point,
      InteractionHandler.centerPointRadius,
      InteractionHandler.centerPointImage,
    );
  }
  ...
  public formatTouchEvent(
    ...
    if (touches && touches.length) {
      for (let i = 0, l = touches.length; i < l; i++) {
        ...
        const pointerId = InteractionHandler.getPointerId(
          e.type,
          touch.identifier,
        );
        ...
        if (event) {
          ...
          if (!invalid) {
            ...
            messages.push(
              ...InteractionHandler.validateMessage(
                e,
                message,
                storage,
                `${logPrefix}[validate]`,
            ...
            if (previous) {
              messages.push(
                InteractionHandler.createEmulatedMessage(
  ...
  public buildTouchEvent(
    ...
    if (hasActionUp && storage.size) {
      console.warn(
        logPrefix,
        "Looks like one of Multi-touch pointers was not raised up",
      );
