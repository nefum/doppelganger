...
export class FeaturedInteractionHandler extends InteractionHandler {
  ...
  private static readonly keyEventsNames: KeyEventNames[] = [
    "keydown",
    "keyup",
  ];
  ...
  public buildScrollEvent(
    event: WheelEvent,
    screenInfo: ScreenInfo,
  ): ScrollControlMessage[] {
    const messages: ScrollControlMessage[] = [];
    const touchOnClient = InteractionHandler.buildTouchOnClient(
      event,
      screenInfo,
    );
    if (touchOnClient) {
      ...
      if (
        ...
        time - this.lastScrollEvent.time >
          FeaturedInteractionHandler.SCROLL_EVENT_THROTTLING_TIME ||
        this.lastScrollEvent.vScroll !== vScroll ||
        this.lastScrollEvent.hScroll !== hScroll
      ) {
        this.lastScrollEvent = { time, hScroll, vScroll };
        messages.push(
          new ScrollControlMessage(
            touchOnClient.touch.position,
            hScroll,
            vScroll,
          ),
        );
