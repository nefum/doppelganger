  ...
  implements TouchHandlerListener
{
  public static calculatePhysicalPoint(
    ...
    const convertedX =
      contentRect.left + (point.x * contentRect.getWidth()) / videoSize.width;
    const convertedY =
      contentRect.top + (point.y * contentRect.getHeight()) / videoSize.height;
  ...
  private wait: Map<
    number,
    { resolve: (m: Message) => void; reject: (error: any) => void }
  > = new Map();
  ...
  public async setMjpegServerOptions(opts: MjpegServerOptions): Promise<void> {
    ...
    if (!mjpegServerFramerate || isNaN(mjpegServerFramerate)) {
      options.mjpegServerFramerate =
        DefaultMjpegServerOption.mjpegServerFramerate;
    }
    ...
    if (!mjpegServerScreenshotQuality || isNaN(mjpegServerScreenshotQuality)) {
      options.mjpegServerScreenshotQuality =
        DefaultMjpegServerOption.mjpegServerScreenshotQuality;
    }
  ...
  public async performClick(position: Position): Promise<void> {
    ...
    const point = WdaProxyClient.calculatePhysicalPoint(
      this.screenInfo,
      screenWidth,
      position,
    );
  ...
  public async performScroll(from: Position, to: Position): Promise<void> {
    ...
    const fromPoint = WdaProxyClient.calculatePhysicalPoint(
      this.screenInfo,
      wdaScreen,
      from,
    );
    const toPoint = WdaProxyClient.calculatePhysicalPoint(
      this.screenInfo,
      wdaScreen,
      to,
    );
  ...
  public async requestWebDriverAgent(
    method: WDAMethod,
    args?: any,
  ): Promise<any> {
    if (!this.hasSession) {
